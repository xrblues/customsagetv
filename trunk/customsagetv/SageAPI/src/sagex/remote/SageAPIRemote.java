...
public class SageAPIRemote implements ISageAPIProvider {
	...
	public Object callService(String name, Object[] args) {
		...
		try {
			...
			urlStr += ("?"+ SageRPCServlet.CMD_ARG +"=" + java.net.URLEncoder.encode(MarshalUtils.marshal(new RemoteRequest(name, args)), MarshalUtils.ENCODING));
			...
			// now check from remote object references... specificlly array ones, and turn those into real arrays...
			...
			if (rdata instanceof RemoteObjectRef && ((RemoteObjectRef)rdata).isArray()) {
				...
				// we need to convert into a local array copy so the local api can deal with it
				...
				// assume the data type was normal, ie not a Sage object of any type
				replyData = rdata;
			}
		} catch (Exception e) {
