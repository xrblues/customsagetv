	...
	public Object extractMetadata(File file, String arg) {
		try {
			...
			if (md==null) {
				...
				List<IMediaSearchResult> results = MediaMetadataFactory.getInstance().search(ConfigurationManager.getInstance().getMetadataConfiguration().getDefaultProviderId(), IMediaMetadataProvider.SEARCH_TITLE, name);
