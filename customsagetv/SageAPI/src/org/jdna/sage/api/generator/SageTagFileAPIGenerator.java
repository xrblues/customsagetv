...
/**
 * Generates a JSP tag file wrapper API for SageTV APIs.  A tag file is created for each function.
 */
public class SageTagFileAPIGenerator {
	...
	public void generate() throws Exception {
	    // Strip off the 'API' suffix and make it lower case.  This becomes the name of the directory that holds
        ...
        for (SageMethod m : methods) {
            ...
            //  * MediaMask - Functions which take a MediaMask are unique because there is also a corresponding
            //                function that does not take a MediaMask.  These are the only overloaded functions
            //                in the SageTV API.  However tag files cannot be overloaded so a separate file
            //                must be created.
            ...
            {
                // Output 'var' attribute is only needed if the SageTV API function returns a value.
            ...
            // Create an optional 'context' attribute on every API.  The caller is responsible for knowing which
            ...
            // Get the tag attributes into Java code so they can be used in the SageTV API call.
            ...
            if (!m.returnType.contains("void"))
            {
                // Just some whitespace for readability
                pw.println("");
                // Set the SageTV API return value on the tag's output attribute.  The value
	...
	/**
	 * SageTV's API does not expose the native object types, everything is an object.  Convert the type names
	 * in the Javadoc to 'Object' types.
	 * @param name
	 * @return
	 */
	...
	/**
	 * Creates the code that represents the object array that is passed to the underlying SageTV API.
	 * @param m
	 * @return
	 */
	...
	/**
	 * Converts a String representing a Java primitive data type to a wrapper object. 
	 * @param o
	 * @return
	 */
	...
	/**
	 * Create the method signature that is printed in the file's comment.  It is used for linking
	 * to SageTV's online Javadoc API.
	 * @param m
	 * @return
	 */
