 ...
 * href='http://download.sage.tv/api/sage/api/Configuration.html'>Configuration</a>
...
public class Configuration {
	...
	/**
	 * Returns all of the library import paths that are currently configured for
	 * all media types
	 * 
	 * Returns: the library import paths that are currently configured for all
	 * media types
	 */
	...
	/**
	 * Returns the library import paths that are currently configured for
	 * picture files
	 * 
	 * Returns: the library import paths that are currently configured for
	 * picture files
	...
	/**
	 * Returns the library import paths that are currently configured for music
	 * files
	 * 
	 * Returns: the library import paths that are currently configured for music
	 * files
	...
	/**
	 * Returns the library import paths that are currently configured for video
	 * files
	 * 
	 * Returns: the library import paths that are currently configured for video
	 * files
	 ...
	 * Deprecated.
	 * 
	 * Returns true if the legacy major/minor channel tuning is enabled. Do not
	 * use anymore
	 * 
	...
	/**
	 * Returns true if SageTV should attempt to autodial before accessing the
	 * Internet
	 * 
	 * Returns: true if SageTV should attempt to autodial before accessing the
	 * Internet
	 ...
	 * 
	 * Parameters: Autodial- true if SageTV should autodial before accessing the
	 * Internet
	 */
	 ...
	 * 
	 * Parameters: Enabled- true if the Provideo 256 Pentium 4 fix should be
	 * enabled
	 */
	...
	/**
	 * Gets the list of infrared tuning plugins that are available for use with
	 * SageTV
	 * 
	 * Returns: the list of infrared tuning plugins that are available for use
	 * with SageTV
	 */
	...
	/**
	 * Gets the names of the 'Remote Controls' which correspond to external
	 * devices that can be controlled with the specified plugin
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin to get the
	 * remotes for PluginPortNumber- the port number of the tuning plugin, 0 if
	 * there is no port for it Returns: the names of the 'Remotes' which
	 * correspond to external devices that can be controlled by the specified
	 * plugin on the specified port
	 */
	...
	/**
	 * Creates a new 'Remote Control' with the specified name for the specified
	 * plugin. Depending upon the plugin, this may require the user to perform
	 * some interaction before it returns.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name to use for the newly created 'Remote
	 * Control' Returns: the RemoteName parameter will be returned if a new
	 * 'Remote Control' is created, null will be returned if one already exists
	 * with that name
	 */
	...
	/**
	 * Informs an infrared tuning plugin that it should go into the learn mode
	 * to learn a new command. This call will return after the user has given
	 * the hardware the appropriate input it is looking for.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' that the command
	 * is being learned from CommandName- the name of the command being sent
	 * from the remote control Returns: true if the command was successfully
	 * learned, false otherwise
	 */
	 ...
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' that the command
	 * is from OldCommandName- the current name of the command NewCommandName-
	 * the new name to rename this command to
	 */
	...
	/**
	 * Removes a 'Remote Control' from an infrared tuning plugin. This will also
	 * delete all commands learned by that 'Remote Control'.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' to remove
	 */
	 ...
	 * If the command had a learned code, that code will be lost.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' CommandName- the
	...
	/**
	 * Sets the maximum number of digits that are needed to tune a channel on
	 * the device that corresponds to the specified 'Remote Control'. For
	 * example, if you always need to push 3 numeric keys before the device
	 * would tune a channel (i.e. channels are from 1-999) then you would
	 * specify 3 here.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' NumberOfDigits-
	 * the maximum number of digits needed to tune a channel using the specified
	 * remote on this plugin
	 */
	...
	/**
	 * Gets the maximum number of digits that are needed to tune a channel on
	 * the device that corresponds to the specified 'Remote Control'. For
	 * example, if you always need to push 3 numeric keys before the device
	 * would tune a channel (i.e. channels are from 1-999) then this should be
	 * 3.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Returns: the
	 * maximum number of digits needed to tune a channel using the specified
	 * remote on this plugin
	 */
	...
	/**
	 * Sets the minimum delay in milliseconds that is required between sending
	 * consecutive commands using the specified 'Remote Control'.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' MsecDelay- the
	 * minimum required delay between commands in milliseconds
	 */
	...
	/**
	 * Gets the minimum delay in milliseconds that is required between sending
	 * consecutive commands using the specified 'Remote Control'.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	...
	/**
	 * Sets the minimum delay in milliseconds that is required between the
	 * completion of a sequence of commands that represent a channel change and
	 * the start of a new sequence of commands that represent a channel change
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' MsecDelay- the
	 * minimum delay in milliseconds between finishing one channel change
	 * sequence and starting another when using the specified 'Remote Control'
	 */
	...
	/**
	 * Gets the minimum delay in milliseconds that is required between the
	 * completion of a sequence of commands that represent a channel change and
	 * the start of a new sequence of commands that represent a channel change
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	...
	/**
	 * Sets the command that is to be sent as the last command to complete a
	 * channel change sequence. This is for devices that require an 'Enter' or
	 * other type of confirmation key to be pressed after the numeric channel
	 * codes are sent.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Command- the
	 * name of the command from this 'Remote Control' to use to confirm a
	 * channel change
	 */
	...
	/**
	 * Sets the command that is to be sent before a channel change sequence is
	 * started. This is for devices that require a 'Power On' or 'Cancel' type
	 * of function to ensure the device is a responsive state.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Command- the
	 * name of the command from this 'Remote Control' to use to prefix a channel
	 * change Since: 6.2
	 */
	...
	/**
	 * Gets the command that is to be sent as the last command to complete a
	 * channel change sequence. This is for devices that require an 'Enter' or
	 * other type of confirmation key to be pressed after the numeric channel
	 * codes are sent.
	 * 
	...
	/**
	 * Gets the command that is to be sent before a channel change sequence is
	 * started. This is for devices that require a 'Power On' or 'Cancel' type
	 * of function to ensure the device is a responsive state.
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	...
	/**
	 * Gets the carrier frequency detected by this plugin for the specified
	 * remote
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Returns: the
	 * carrier frequency of the specified 'Remote Control', or zero if the
	 * plugin doesn't have that information
	 */
	 ...
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Returns: the
	 * 'bit time' of the specified 'Remote Control', or zero if the plugin
	...
	/**
	 * Gets the list of commands for the specified 'Remote Control' for the
	 * specified plugin
	 * 
	 * Parameters: PluginName- the name of the infrared tuning plugin
	 * PluginPortNumber- the port number of the tuning plugin, 0 if there is no
	 * port for it RemoteName- the name of the 'Remote Control' Returns: the
	 * list of commands for the specified 'Remote Control' for the specified
	 * plugin
	 */
	...
	/**
	 * Returns the list of properties underneath the specified property that
	 * themselves contain further subproperties. For example, if the following
	 * were the properties: animal/pig=0 animal/fish/salmon=1
	 * animal/fish/trout=2 animal/reptile/green/alligator=4
	 * 
	 * and this call was made with "animal" as the argument, then the returned
	 * values would be "fish" and "reptile"
	 * 
	 * Parameters: PropertyName- the root property to check for subproperty
	 * branches under Returns: the list of properties underneath the specified
	 * property that themselves contain further subproperties.
	 */
	...
	/**
	 * Returns the list of properties underneath the specified property that do
	 * not contain further subproperties. For example, if the following were the
	 * properties: animal/pig=0 animal/fish/salmon=1 animal/fish/trout=2
	 * animal/reptile/green/alligator=4
	 * 
	 * and this call was made with "animal" as the argument, then the returned
	 * values would only be "pig"
	 * 
	 * Parameters: PropertyName- the root property to check for subproperty
	 * leaves under Returns: the list of properties underneath the specified
	 * property that do not contain further subproperties
	 */
	 ...
	 * 
	 * and this call was made with "animal" as the argument, then the returned
	 * values would be "fish" and "reptile" If this is called from a client
	 * instance then it will use the properties on the server system for this
	 * call.
	 * 
	 * Parameters: PropertyName- the root property to check for subproperty
	 * branches under Returns: the list of properties underneath the specified
	 * property that themselves contain further subproperties. Since: 6.1
	 */
	 ...
	 * 
	 * and this call was made with "animal" as the argument, then the returned
	 * values would only be "pig" If this is called from a client instance then
	 * it will use the properties on the server system for this call.
	 * 
	 * Parameters: PropertyName- the root property to check for subproperty
	 * leaves under Returns: the list of properties underneath the specified
	 * property that do not contain further subproperties Since: 6.1
	 */
	...
	/**
	 * Gets the property with the specified name. If the property is not set
	 * yet, it will be set to the DefaultValue that is passed in unless that
	 * value is null
	 * 
	 * Parameters: PropertyName- the name of the property to retrieve
	 * DefaultValue- the default value to set the property to if it does not
	 * exist, null if it should not be set Returns: the value of the specified
	 * property, or the DefaultValue if the property was not set prior to this
	 * call
	 */
	 ...
	 * 
	 * Parameters: PropertyName- the name of the property to set PropertyValue-
	 ...
	 * Gets the property with the specified name. If the property is not set
	 * yet, it will be set to the DefaultValue that is passed in unless that
	 * value is null. If this is called from a client instance then it will use
	 * the properties on the server system for this call.
	 * 
	 * Parameters: PropertyName- the name of the property to retrieve
	 * DefaultValue- the default value to set the property to if it does not
	 * exist, null if it should not be set Returns: the value of the specified
	...
	/**
	 * Sets the property with the specified name to the specified value. If this
	 * is called from a client instance then it will use the properties on the
	 * server system for this call and the change will be made on the server
	 * system.
	 * 
	 * Parameters: PropertyName- the name of the property to set PropertyValue-
	 * the value to set the property to
	...
	/**
	 * Removes the specified property and any subproperties that start with the
	 * same prefix
	 * 
	 * Parameters: PropertyName- the name of the property that should be removed
	 * along with all of its subproperties
	 */
	...
	/**
	 * Removes the specified property from the property map. If this is called
	 * from a client instance then it will use the properties on the server
	 * system for this call and the change will be made on the server system.
	 * 
	 * Parameters: PropertyName- the name of the property to remove Since: 6.1
	 */
	...
	/**
	 * Removes the specified property and any subproperties that start with the
	 * same prefix. If this is called from a client instance then it will use
	 * the properties on the server system for this call and the change will be
	 * made on the server system.
	 * 
	 * Parameters: PropertyName- the name of the property that should be removed
	 * along with all of its subproperties Since: 6.1
	 */
	 ...
	 * 
	 * Returns: the name of the default quality that is used to record
	 * television
	 */
	...
	/**
	 * Returns true if the 'Intelligent Recording' feature of SageTV is
	...
	/**
	 * Adds a new file path to the list of paths that SageTV checks for library
	 * import files. This path will be used to search for music, video and
	 * picture files.
	 * 
	 * Parameters: NewPath- the file path to add to the list of library import
	 * paths
	 */
	...
	/**
	 * Adds a new file path to the list of paths that SageTV checks for library
	 * import files. This path will be used to search for picture files only.
	 * 
	 * Parameters: NewPath- the file path to add to the list of library import
	...
	/**
	 * Adds a new file path to the list of paths that SageTV checks for library
	 * import files. This path will be used to search for music files only.
	 * 
	 * Parameters: NewPath- the file path to add to the list of library import
	 * paths for music files
	 */
	...
	/**
	 * Adds a new file path to the list of paths that SageTV checks for library
	 * import files. This path will be used to search for video files only.
	 * 
	 * Parameters: NewPath- the file path to add to the list of library import
	...
	/**
	 * Removes a path from the list of library import paths. This will affect
	 * all import types.
	 * 
	 * Parameters: RemovePath- the path to remove from the list of library
	 * import paths
	 */
	 ...
	 * 
	 * Parameters: Quality- the name of the quality setting to use to record
	 * television by default
	 */
	 ...
	 * 
	 * Parameters: Disabled- should be true to disable 'Intelligent Recording',
	 * false to enable it
	 */
	 ...
	 * Deprecated.
	 * 
	 * Sets whether or not SageTV should use legacy major-minor DTV channels.
	 * Not recommend for use anymore.
	 * 
	 * Parameters: Enabled- true if SageTV should use legacy major-minor DTV
	 * channel tuning
	 */
	...
	/**
	 * Returns true if SageTV will go to its screen saver when it is put to
	 * sleep. If SageTV is not in full screen mode then it will go to the system
	 * tray regardless of this setting.
	 * 
	 * Returns: true if SageTV will go to its screen saver when put to sleep,
	 * false otherwise
	 */
	...
	/**
	 * Sets whether or not SageTV should go to its screen saver when put to
	 * sleep. If SageTV is not in full screen mode then it will go to the system
	 * tray regardless of this setting.
	 * 
	 * Parameters: Enabled- true if SageTV should go to the screen saver when
	 * put to sleep, false otherwise
	 */
	...
	/**
	 * Returns true if SageTV should synchronize the system clock with the
	 * server. For SageTV Client this will synchronize with its SageTV Server.
	 * For a SageTV system, this will synchronize with the EPG data server.
	 * 
	 * Returns: true if SageTV should synchronize the system clock with the
	 * server
	 */
	...
	/**
	 * Sets if SageTV should synchronize the system clock with the server. For
	 * SageTV Client this will synchronize with its SageTV Server. For a SageTV
	 * system, this will synchronize with the EPG data server.
	 * 
	 * Parameters: Enabled- true if SageTV should synchronize the system clock
	 * with the server
	 */
	 ...
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 ...
	 * 
	 * Returns: the list of localized names for the aspect ratio modes
	 */
	...
	/**
	 * Returns the display aspect ratio that is set for the current display
	 * device being used
	 * 
	 * Returns: the display aspect ratio that is set for the current display
	 * device being used Since: 5.1
	 */
	 ...
	 * 
	 * Parameters: DisplayAspectRatio- the aspect ratio of the current display
	 * device Since: 5.1
	 */
	 ...
	 * 
	 * Parameters: Enabled- true if parental controls should be enabled, false
	 * otherwise
	 */
	...
	/**
	 * Returns true if SageTV should mute the volume when media is played back
	 * at speeds besides x1
	 * 
	 * Returns: true if SageTV should mute the volume when media is played back
	 * at speeds besides x1, false otherwise
	 */
	...
	/**
	 * Sets whether or not SageTV should mute the sound when media is played
	 * back at a speed other than x1
	 * 
	 * Parameters: Enabled- true if SageTV should mute the sound when media is
	 * played at a speed other x1, false otherwise
	 */
	 ...
	 * 
	 * Returns: true if a parental lock code has been configured, false
	 * otherwise
	 */
	...
	/**
	 * Gets the list of content ratings that are under parental lock. When any
	 * content that has ratings that match anything in the restricted list is
	 * requested for playback; the parental lock code must be entered before
	 * playback will begin.
	 * 
	 * Returns: the list of content ratings that are under parental lock
	 */
	...
	/**
	 * Returns true if the specified rating is in the list that is under
	 * parental control
	 * 
	 * Returns: true if the specified rating is in the list that is under
	 * parental control, false otherwise
	 */
	...
	/**
	 * Returns the color that should be used for the video background. This is
	 * only shown when the video's aspect ratio doesn't match the area that it's
	 * playing back in.
	 * 
	...
	/**
	 * Sets the color that should be used for the video background. This is only
	 * shown when the video's aspect ratio doesn't match the area that it's
	 * playing back in.
	 * 
	 * Parameters: Color- the color that should be used for the video background
	 */
	...
	/**
	 * Gets the video orbiting duration in milliseconds. This is used to shift
	 * the video left-to-right very slowly over time. It is used on widescreen
	 * displays to prevent screen burn when watching 4:3 content.
	 * 
	 * Returns: the video orbiting duration in milliseconds
	 */
	...
	/**
	 * Sets the video orbiting duration in milliseconds. This is used to shift
	 * the video left-to-right very slowly over time. It is used on widescreen
	 * displays to prevent screen burn when watching 4:3 content.
	 * 
	 * Parameters: Duration- the video orbiting duration in milliseconds
	 */
	...
	/**
	 * Gets the horizontal scale factor for the video for the specified aspect
	 * ratio mode.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * Returns: the horizontal scale factor for the video for the specified
	 * aspect ratio mode
	 */
	...
	/**
	 * Sets the horizontal scale factor for the video for the specified aspect
	 * ratio mode.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * ScaleFactor- the horizontal scale factor for the video for the specified
	 * aspect ratio mode
	 */
	...
	/**
	 * Gets the vertical scale factor for the video for the specified aspect
	 * ratio mode.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * Returns: the vertical scale factor for the video for the specified aspect
	 * ratio mode
	 */
	...
	/**
	 * Sets the vertical scale factor for the video for the specified aspect
	 * ratio mode.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * ScaleFactor- the vertical scale factor for the video for the specified
	 * aspect ratio mode
	 */
	...
	/**
	 * Gets of the offset in pixels that should be applied horizontally to the
	 * video. Positive numbers move it to the right.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * Returns: the offset in pixels that should be applied horizontally to the
	 * video
	 */
	...
	/**
	 * Sets of the offset in pixels that should be applied horizontally to the
	 * video. Positive numbers move it to the right.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * PixelOffset- the offset in pixels that should be applied horizontally to
	 * the video
	 */
	...
	/**
	 * Gets of the offset in pixels that should be applied vertically to the
	 * video. Positive numbers move it down.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * Returns: the offset in pixels that should be applied vertically to the
	 * video
	 */
	...
	/**
	 * Sets of the offset in pixels that should be applied vertically to the
	 * video. Positive numbers move it down.
	 * 
	 * Parameters: AspectRatioMode- the localized name of the aspect ratio mode
	 * PixelOffset- the offset in pixels that should be applied vertically to
	 * the video
	 */
	...
	/**
	 * Sets the code that is to be used in order to unlock the parental
	 * controls.
	 * 
	 * Parameters: ParentalLockCode- the new value to set the parental lock code
	 * to
	 */
	...
	/**
	 * Returns a list of the different recording quality names available in the
	 * system
	 * 
	 * Returns: a list of the different recording quality names available in the
	 * system
	...
	/**
	 * Gets the estimated overall bitrate for a given recording quality. The
	 * returned value is in Megabits per second.
	 * 
	 * Parameters: Quality- the name of the recording quality Returns: the
	 * estimated overall bitrate for the specified recording quality in Megabits
	 * per second
	 */
	...
	/**
	 * Gets the format for a given recording quality. Returns values like
	 * "MPEG-2 PS", "DivX", etc.
	 * 
	 * Parameters: Quality- the name of the recording quality Returns: the
	 * format for the specified recording quality
	 */
	...
	/**
	 * Gets the list of directories that SageTV is configured to record
	 * television to
	 * 
	 * Returns: the list of directories that SageTV is configured to record
	 * television to
	 */
	...
	/**
	 * Returns the diskspace rule that SageTV should use when managing the disk
	 * space in a given directory. The names for the rules are localized
	 * versions of the strings "Use Only", "Use All" and "Leave Free"
	 * 
	 * Parameters: Directory- the video directory to get the diskspace rule for
	 * Returns: the diskspace rule for the specified video directory
	 */
	...
	/**
	 * Returns the size (in bytes) that should be applied to the specified
	 * directory for its configured diskspace rule
	 * 
	 * Parameters: Directory- the video directory to get the size for that
	 * applies to its rule Returns: the size (in bytes) that should be applied
	 * to the specified directory for its configured diskspace rule
	 */
	...
	/**
	 * Removes a directory from SageTV's list of directories that it can record
	 * television to. If you delete all of the directories; SageTV will be
	 * unable to record television. This does NOT delete the directory or any of
	 * its files from disk. They will still be fully accessible in SageTV.
	 * 
	 * Parameters: Directory- the directory that SageTV should no longer record
	...
	/**
	 * Adds a new directory to SageTV's list of directories that it can record
	 * video to. Also specifies what rule & size to use when managing the
	 * diskspace for this directory.
	 * 
	 * Parameters: Directory- the directory to add as a video directory Rule-
	 * the diskspace rule to use for this directory seeGetVideoDiskspaceRules()
	 * 
	...
	/**
	 * Changes the location of a given video directory, or modifies the
	 * diskspace rule or diskspace rule size for a video directory.
	 * 
	 * Parameters: OldDirectory- the currently configured path for the video
	 * directory that is to have its path or rules modified NewDirectory- the
	 * new file path to use for the video directory (this can be the same as
	 * OldDirectory) NewRule- the diskspace rule to use for this video
	 * directory, seeGetVideoDiskspaceRules()
	 * 
	...
	/**
	 * Returns the list of valid diskspace rules. Will be localized versions of
	 * the strings "Use All", "Use Only" and "Leave Free"
	 * 
	 ...
	 * When one of these IR codes is received; that will cause the specified
	 * SageTV Command to get executed.
	 * 
	 * Parameters: SageCommand- the SageTV Command for which the IR codes should
	 * be retrieved Returns: the infrared codes that are linked to the specified
	 * SageTV Command
	 */
	 ...
	 * When one of these keystrokes is received; that will cause the specified
	 * SageTV Command to get executed.
	 * 
	 * Parameters: SageCommand- the SageTV Command for which the keystrokes
	 * should be retrieved Returns: the keystrokes that are linked to the
	 * specified SageTV Command
	 */
	...
	/**
	 * Gets the name that is associated with an IR code. This has no functional
	 * purpose and is purely for display purposes.
	 * 
	 * Parameters: IRCode- the IRCode to get the associated name for,
	 * seeSetNameForIRCode()
	 */
	 ...
	 * 
	 * Parameters: IRCode- the infrared code Returns: the SageTV Command that is
	 * linked to the specified infrared code
	 */
	 ...
	 * 
	 * Parameters: Keystroke- the name of the keystroke Returns: the SageTV
	 * Command that is linked to the specified keystroke
	 */
	...
	/**
	 * Sets the name for a given IR code. This has no functional purpose, and is
	 * purely for display purposes.
	 * 
	 * Parameters: IRCode- the infrared code to attach a name to Name- the name
	 * to attach to the specified infrared code
	 */
	...
	/**
	 * Links an infrared code to a SageTV Command. The specified SageTV Command
	 * will then be executed whenever the specified IRCode is received.
	 * 
	 * Parameters: IRCode- the infrared code to link to the SageTV Command
	...
	/**
	 * Links a keystroke to a SageTV Command. The specified SageTV Command will
	 * then be executed whenever the specified keystroke is received.
	 * 
	 * Parameters: Keystroke- the name of the keystroke to link to the SageTV
	 * Command SageCommand- the SageTVCommand to link the keystroke to
	 */
	 ...
	 * 
	 * Parameters: IRCode- the infrared code to unlink from any SageTV Commands
	 ...
	 * 
	 * Parameters: Keystroke- the keystroke to unlink from any SageTV Commands
	 * that it is linked with
	 */
	...
	/**
	 * Sets whether or not a given rating should be restricted under parental
	 * control
	 * 
	 * Parameters: Rating- the name of the rating Restricted- true if the
	 * specified Rating should be restricted by parental control, false
	 * otherwise
	 */
	 ...
	 * 
	 * Returns: the name of the port that the infrared receiver is connected to;
	 * will be a value like: "COM1", "COM2" or "USB"
	 */
	 ...
	 * 
	 * Parameters: Port- the name of the port that the infrared receiver is
	 * connected to; should be a value like: "COM1", "COM2" or "USB"
	 */
	...
	/**
	 * Gets the inactivity timeout in milliseconds. The hook InactivityTimeout()
	 * will be called whenever the system experiences inactivity for this amount
	 * of time.
	 * 
	...
	/**
	 * Sets the inactivity timeout in milliseconds. The hook InactivityTimeout()
	 * will be called whenever the system experiences inactivity for this amount
	 * of time.
	 * 
	 * Parameters: Timeout- the inactivity timeout in milliseconds
	 */
	...
	/**
	 * Gets the screen saver timeout in milliseconds. The "Screen Saver" menu
	 * will be launched after this period of user inactivity if the program is
	 * in full screen mode.
	 * 
	 * Returns: the screen saver timeout in milliseconds
	 */
	...
	/**
	 * Sets the screen saver timeout in milliseconds. The "Screen Saver" menu
	 * will be launched after this period of user inactivity if the program is
	 * in full screen mode.
	 * 
	 * Parameters: Timeout- the screen saver timeout in milliseconds
	 */
	...
	/**
	 * Returns true if the SageTV server is enabled. This allows SageTV Clients
	 * to connect.
	 * 
	 * Returns: true if the SageTV server is enabled, false otherwise
	 */
	...
	/**
	 * Sets whether or not the SageTV server should be enabled. This call will
	 * take effect immediately (i.e. the server can be started/stopped on
	 * demand)
	 * 
	 * Parameters: Enabled- true if the SageTV server should be enabled, false
	 * otherwise
	 */
	...
	/**
	 * Returns true if 3D acceleration is enabled for the rendering engine. This
	 * uses DirectX9-3D on Windows and OpenGL on Linux.
	 * 
	...
	/**
	 * Specifies whether or not 3D acceleration should be enabled for the
	 * rendering engine. This uses DirectX9 on Windows and OpenGL on Linux. This
	 * call takes effect immediately.
	 * 
	 * Parameters: Enabled- true if 3D acceleration should be enabled, false
	 * otherwise.
	 */
	...
	/**
	 * Returns the horizontal offset in pixels that should be applied to the UI
	 * to account for overscan. This moves the UI to fit on a TV screen in the
	 * case where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Returns: the horizontal offset in pixles that should be applied to the UI
	 * to account for overscan, positive values move it to the right
	 */
	...
	/**
	 * Returns the vertical offset in pixels that should be applied to the UI to
	 * account for overscan. This moves the UI to fit on a TV screen in the case
	 * where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Returns: the vertical offset in pixles that should be applied to the UI
	 * to account for overscan, positive values move it down
	 */
	...
	/**
	 * Returns the horizontal scale factor that should be applied to the UI to
	 * account for overscan. This shrinks the UI to fit on a TV screen in the
	 * case where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Returns: the horizontal scale factor that should be applied to the UI to
	...
	/**
	 * Returns the vertical scale factor that should be applied to the UI to
	 * account for overscan. This shrinks the UI to fit on a TV screen in the
	 * case where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	...
	/**
	 * Sets the horizontal offset in pixels that should be applied to the UI to
	 * account for overscan. This moves the UI to fit on a TV screen in the case
	 * where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Parameters: Amount- the horizontal offset in pixles that should be
	 * applied to the UI to account for overscan, positive values move it to the
	 * right
	 */
	...
	/**
	 * Sets the vertical offset in pixels that should be applied to the UI to
	 * account for overscan. This moves the UI to fit on a TV screen in the case
	 * where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Parameters: Amount- the vertical offset in pixles that should be applied
	 * to the UI to account for overscan, positive values move it down
	 */
	...
	/**
	 * Sets the horizontal scale factor that should be applied to the UI to
	 * account for overscan. This shrinks the UI to fit on a TV screen in the
	 * case where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Parameters: Amount- the horizontal scale factor that should be applied to
	 * the UI to account for overscan
	 */
	...
	/**
	 * Sets the vertical scale factor that should be applied to the UI to
	 * account for overscan. This shrinks the UI to fit on a TV screen in the
	 * case where it goes off the edges of the screen due to what is known as
	 * 'Overscan'.
	 * 
	 * Parameters: Amount- the vertical scale factor that should be applied to
	 * the UI to account for overscan
	 */
	...
	/**
	 * Gets the names of the countries that can be used in theSetCountry() call.
	 * 
	 * Returns: the possible names for country selection
	 */
	...
	/**
	 * Sets the country that the user is in. This is used for determining the
	 * frequencies used for tuning television signals.
	 * 
	 * Parameters: Country- the name of the country the user is in, should be a
	 * value fromGetAvailableCountries()
	 * 
	 * Returns: the passed in Country variable
	 */
	...
	/**
	 * Gets the country that SageTV is configured for. This is used for
	 * determining the frequencies used for tuning television signals.
	 * 
	 * Returns: the name of the country that SageTV is configured for
	 */
	...
	/**
	 * Sets the analog video format that should be used for capture and
	 * playback. Valid values are "NTSC" or "PAL"
	 * 
	 * Parameters: Format- the analog video format that should be used for
	 * capture and playback; valid values are "NTSC" or "PAL"
	 */
	...
	/**
	 * Gets the analog video format that should be used for capture and
	 * playback. Valid values are "NTSC" or "PAL"
	 * 
	 * Returns: the analog video format that should be used for capture and
	 * playback
	 */
	 ...
	 * 
	 * Returns: the language that the user interface should be displayed in
	 */
	...
	/**
	 * Sets the language that the user interface should be displayed in. This
	 * takes effect immediately. Use theGetAvailableUILanguages() call to get
	 * the options for this.
	 * 
	 * Parameters: Language- the language that the user interface should be
	 * displayed in
	 */
	...
	/**
	 * Gets the list of languages that are available to be used for the user
	 * interface.
	 * 
	 * Returns: the list of languages that are available to be used for the user
	 * interface
	 */
	...
	/**
	 * Gets the name of the time zone that the application is currently
	 * configured to use
	 * 
	 * Returns: the name of the time zone that the application is currently
	 * configured to use
	 */
	...
	/**
	 * Sets the name of the time zone that the application should use. This
	 * takes effect immediately.
	 * 
	 * Parameters: TimeZone- tha name of the time zone to use,
	 * seeGetAllTimeZones()
	 */
	 ...
	 * 
	 * Returns: the list of all of the valid time zone names
	 */
	...
	/**
	 * Gets the names of the regions for the configured country which are used
	 * for dividing it up into different areas for DVB-T reception. Returns a
	 * zero-length list if there are no sub-regions within the country. Use the
	 * results from this in a call toSetDVBTRegion(DVBTRegion)
	 * 
	 * 
	 * Returns: the region names in the country to use for DVBT reception
	 * differentiation Since: 5.1
	 */
	 ...
	 * 
	 * Parameters: DVBTRegion- the name of the region the user is in, should be
	 * a value fromGetDVBTRegions()
	 * 
	 * Since: 5.1
	 */
	...
	/**
	 * Gets the the region within the current country that the user is in for
	 * DVB-T tuning purposes. Must have been set with a call
	 * toSetDVBTRegion(DVBTRegion)
	 * 
	 * 
	 * Returns: the name of the region the user is in Since: 5.1
	 */
	...
	/**
	 * Gets the names of the DVB-C providers for the configured country which
	 * are used for dividing it up into different areas for DVB-C reception.
	 * Returns a zero-length list if there are one or no providers in this
	 * country. Use the results from this in a call toSetDVBCRegion(DVBCRegion)
	 * 
	 * 
	 * Returns: the provider names in the country to use for DVBC reception
	 * differentiation Since: 5.1
	 */
	...
	/**
	 * Sets the the provider within the current country that the user has for
	 * DVBC reception.
	 * 
	 * Parameters: DVBCRegion- the name of the provider the user has, should be
	 * a value fromGetDVBCRegions()
	 * 
	 * Since: 5.1
	 */
	...
	/**
	 * Gets the the provider within the current country that the user has for
	 * DVB-C tuning purposes. Must have been set with a call
	 * toSetDVBCRegion(DVBCRegion)
	 * 
	 * 
	 * Returns: the name of the provider the user has for DVB-C Since: 5.1
	 */
	...
	/**
	 * Gets the names of the providers for the configured country which are used
	 * for dividing it up into different areas for DVB-S reception. Returns a
	 * zero-length list if there are one or no providers within the country. Use
	 * the results from this in a call toSetDVBSRegion(DVBSRegion)
	 * 
	 * 
	 * Returns: the providers names in the country to use for DVBS reception
	 * differentiation Since: 5.1
	 */
	 ...
	 * DVBS reception.
	 * 
	 * Parameters: DVBSRegion- the name of the provider the user has, should be
	 * a value fromGetDVBSRegions()
	 * 
	 * Since: 5.1
	 */
	...
	/**
	 * Gets the the provider within the current country that the user has for
	 * DVB-S tuning purposes. Must have been set with a call
	 * toSetDVBSRegion(DVBSRegion)
	 * 
	 * 
	 * Returns: the name of the provider the user has for DVB-S Since: 5.1
	 */
	...
	/**
	 * Returns true if this device can do native output resolutions (where
	 * output resolution matches the format of the media being played)
	 * 
	 * Returns: true if this device can do native output resolutions Since: 6.3
	 */
	...
	/**
	 * Returns true if this device is configured to match the output resolution
	 * to the media format's resolution
	 * 
	 * Returns: true if this device is configured to match the output resolution
	 * to the media format's resolution Since: 6.3
	 */
	...
	/**
	 * Sets whether or not this device should try to match its output resolution
	 * to the media format's resolution
	 * 
	 * Parameters: Enabled- true if this device should be configured to match
	 * the output resolution to the media format's resolution; false to use a
	 * fixed resolution Since: 6.3
	 */
	...
	/**
	 * Returns a string describing the currently selected audio output (empty
	 * string if this can't be configured)
	 * 
	 * Returns: a string describing the currently selected audio output (empty
	 * string if this can't be configured) Since: 6.3
	 */
	 ...
	 * 
	 * Returns: an array of the audio outputs that can be used, null if this
	 * change is not supported Since: 6.3
	 */
	...
	/**
	 * Sets the current audio output. This is currently only used on media
	 * extender devices that have configurable audio outputs.
	 * 
	 * Parameters: AudioOutput- this must be a value fromGetAudioOutputOptions()
	 * and should be the new desired audio output Since: 6.3
	 */
	...
	/**
	 * Returns a string describing the type of connection autodetected on HDMI
	 * (currently only used on media extenders)
	 * 
	 * Returns: a string describing the type of connection autodetected on HDMI
	 * (empty string if this isn't supported) Since: 6.3
	 */
	 ...
	 * Deprecated.
	 * 
	 * Sets the DirectShow audio & video decoders on windows to accepetable
	 * values
	 */
	 ...
	 * This will be A, B, C or D (or Default if not set) This is used when
	 * playing back MPEG2 files.
	 * 
	 * Returns: the preferred DXVA MPEG Mode configuration setting
	 */
	 ...
	 * This will be A, B, C or D. This is used when playing back MPEG2 files.
	 * 
	 * Parameters: DXVAMode- the preferred DXVA MPEG Mode configuration setting,
	 * use the empty string or "Default" to disable this
	 */
	...
	/**
	 * Sets the preferred DXVA MPEG Mode configuration setting (Windows only).
	 * This will be A, B, C or D. This is used when playing back DVDs.
	 * 
	 * Parameters: DXVAMode- the preferred DXVA MPEG Mode configuration setting,
	 * use the empty string or "Default" to disable this
	 */
	...
	/**
	 * Gets the list of available DXVA MPEG Modes. This will return {"A", "B",
	 * "C", "D"}
	 * 
	 * Returns: the list of available DXVA MPEG Modes: {"A", "B", "C", "D"}
	 */
	...
	/**
	 * Gets the preferred DXVA Deinterlacing configuration setting (Windows
	 * only). This is used when playing back MPEG2 files.
	 * 
	 * Returns: the preferred DXVA Deinterlacing configuration setting
	 */
	...
	/**
	 * Sets the preferred DXVA Deinterlacing configuration setting (Windows
	 * only). This is used when playing back MPEG2 files.
	 * 
	 * Parameters: DeinterlaceMode- the preferred DXVA Deinterlacing
	...
	/**
	 * Gets the preferred DXVA Deinterlacing configuration setting (Windows
	 * only). This is used when playing back DVDs.
	 * 
	 * Returns: the preferred DXVA Deinterlacing configuration setting
	 */
	 ...
	 * 
	 * Returns: the list of potential DXVA Deinterlacing Capabilities
	 */
	...
	/**
	 * Gets the mode that DScaler is currently configured to run in when it is
	 * used (Windows only)
	 * 
	 * Returns: the mode that DScaler is currently configured to run in when it
	 * is used
	 */
	...
	/**
	 * Sets the mode that DScaler should be configured to run in when it is used
	 * (Windows only)
	 * 
	 * Parameters: DscalerMode- the mode that DScaler should be configured to
	 * run in when it is used
	 */
	 ...
	 * 
	 * Returns: the list of the available modes that DScaler can be run in
	 */
	...
	/**
	 * Gets the name of the DirectShow audio decoder filter that's used for
	 * MPEG2 playback (Windows only)
	 * 
	 * Returns: the name of the DirectShow audio decoder filter that's used for
	 * MPEG2 playback
	 */
	...
	/**
	 * Sets the name of the DirectShow audio decoder filter that's used for
	 * MPEG2 playback (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow audio decoder filter
	 * to use for MPEG2 playback
	 */
	...
	/**
	 * Gets the name of the DirectShow audio decoder filter that's used for DVD
	...
	/**
	 * Sets the name of the DirectShow audio decoder filter that's used for DVD
	 * playback (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow audio decoder filter
	 * to use for DVD playback
	 */
	...
	/**
	 * Gets the name of the DirectShow audio renderer filter that's used for all
	 * playback except DVDs (Windows only)
	 * 
	 * Returns: the name of the DirectShow audio renderer filter that's used for
	 * playback except DVDs
	 */
	...
	/**
	 * Sets the name of the DirectShow audio renderer filter that's used for
	 * playback except DVDs (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow audio renderer filter
	 * to use for playback except DVDs
	 */
	...
	/**
	 * Gets the name of the DirectShow audio renderer filter that's used for DVD
	 * playback (Windows only)
	 * 
	 * Returns: the name of the DirectShow audio renderer filter that's used for
	 * DVD playback
	...
	/**
	 * Sets the name of the DirectShow audio renderer filter that's used for DVD
	 * playback (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow audio renderer filter
	 * to use for DVD playback
	 ...
	 * Possible values are "VMR9", "Overlay" and the localized "Default" string
	 * 
	 * Returns: the video renderer filter that should be used for DVD playback
	 */
	 ...
	 * Possible values are "VMR9", "Overlay" and the localized "Default" string
	 * 
	 * Parameters: FilterName- the video renderer filter that should be used for
	 * DVD playback
	 */
	...
	/**
	 * Gets the name of the DirectShow video decoder filter that's used for
	 * MPEG2 playback (Windows only)
	 * 
	 * Returns: the name of the DirectShow video decoder filter that's used for
	 * MPEG2 playback
	 */
	...
	/**
	 * Sets the name of the DirectShow video decoder filter that's used for
	 * MPEG2 playback (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow video decoder filter
	 * to use for MPEG2 playback
	 */
	...
	/**
	 * Gets the name of the DirectShow video decoder filter that's used for DVD
	 * playback (Windows only)
	 * 
	 * Returns: the name of the DirectShow video decoder filter that's used for
	 * DVD playback
	...
	/**
	 * Sets the name of the DirectShow video decoder filter that's used for DVD
	 * playback (Windows only)
	 * 
	 * Parameters: FilterName- the name of the DirectShow video decoder filter
	 * to use for DVD playback
	...
	/**
	 * Gets the audio delay in milliseconds to apply when playing back MPEG2
	 * files (Windows only)
	 * 
	 * Returns: the audio delay in milliseconds to apply when playing back MPEG2
	 * files
	 */
	...
	/**
	 * Sets the audio delay in milliseconds to apply when playing back MPEG2
	 * files (Windows only)
	 * 
	 * Parameters: Delay- the audio delay in milliseconds to apply when playing
	 * back MPEG2 files
	 */
	...
	/**
	 * Gets the list of the audio decoder filters that are available in the
	 * system for MPEG2 playback (Windows only)
	 * 
	 * Returns: the list of the audio decoder filters that are available in the
	 * system for MPEG2 playback
	 */
	...
	/**
	 * Gets the list of the video decoder filters that are available in the
	 * system for MPEG2 playback (Windows only)
	 * 
	 * Returns: the list of the video decoder filters that are available in the
	 * system for MPEG2 playback
	 */
	...
	/**
	 * Returns true if software decoding is disabled. This only applies to the
	 * TV output of the Hauppauge WinTV PVR-350 PCI card on Windows
	 * 
	...
	/**
	 * Sets if software decoding is disabled. This only applies to the TV output
	 * of the Hauppauge WinTV PVR-350 PCI card on Windows
	 * 
	 * Parameters: Disabled- true if software decoding is disabled
	 */
	...
	/**
	 * Returns true if DScaler is set to double the refresh rate when used. This
	 * means it'll run at 60fps instead of 30fps.
	 * 
	...
	/**
	 * Sets whether or not DScaler should double the refresh rate. This means
	 * it'll run at 60fps instead of 30fps.
	 * 
	 * Parameters: Enabled- true if DScaler should double the refresh rate when
	...
	/**
	 * Returns true if DScaler is configured to display the odd field first
	 * (fields are even or odd)
	 * 
	 * Returns: true if DScaler is configured to display the odd field first
	 */
	...
	/**
	 * Sets if DScaler should display the odd field first (fields are even or
	 * odd)
	 * 
	 * Parameters: Enabled- true if DScaler should display the odd field first
	 */
	...
	/**
	 * Returns true if the system has a Hauppauge WinTV PVR-350 card installed
	 * which can produce a TV output (Windows only)
	 * 
	 * Returns: true if the system has a Hauppauge WinTV PVR-350 card installed
	 * which can produce a TV output
	 */
	...
	/**
	 * Returns true if the TV output on a Hauppauge WinTV PVR-350 card is
	...
	/**
	 * Sets whether or not the TV output on a Hauppauge WinTV PVR-350 should be
	 * enabled (Windows only)
	 * 
	 * Parameters: Enabled- true if the TV output of a Hauppauge WinTV PVR-350
	 * should be enabled
	 */
	...
	/**
	 * Returns true if the user interface (OSD) display on a Hauppauge WinTV
	 * PVR-350 should be shown through the tv output (Windows only)
	 * 
	 * Returns: true if the user interface (OSD) display on a Hauppauge WinTV
	 * PVR-350 should be shown through the tv output
	 */
	...
	/**
	 * Sets whether or not the user interface (OSD) should be displayed on the
	 * TV output of a Hauppauge WinTV PVR-350 (Windows only)
	 * 
	 * Parameters: Enabled- true if the user interface (OSD) should be displayed
	 * on the TV output of a Hauppauge WinTV PVR-350, false otherwise
	 */
	...
	/**
	 * Returns true if the DScaler deinterlacing filter is enabled (Windows
	 * only)
	 * 
	 * Returns: true if the DScaler deinterlacing filter is enabled
	 */
	...
	/**
	 * Sets whether or not the DScaler deinterlacing filter should be used
	 * (Windows only)
	 * 
	 * Parameters: Enabled- true if the DScaler deinterlacing filter should be
	 * used, false otherwise
	 */
	...
	/**
	 * Gets the plugin that DScaler is currently configured to use when it is in
	 * "Plugin" mode.
	 * 
	 * Returns: the plugin that DScaler is currently configured to use when it
	 * is in "Plugin" mode
	 */
	...
	/**
	 * Sets the plugin that should be used by DScaler. DScaler must also be set
	 * into "Plugin" mode.
	 * 
	 * Parameters: PluginName- the name of the plugin to use with DScaler
	 */
	...
	/**
	 * Returns the list of plugins that are supported by DScaler. These are used
	 * when DScaler is in "Plugin" mode. (Windows only)
	 * 
	 ...
	 * 
	 * Returns: true if the system has DirectX 9 installed, false otherwise
	 */
	...
	/**
	 * Returns true if the video renderer is configured to use the Overlay
	 * surface (Windows Only) Applies to all files except DVDs.
	 * 
	 * Returns: true if the video renderer is configured to use the Overlay
	...
	/**
	 * Returns true if the video renderer is configured to use the Video Mixing
	 * Renderer 9 (Windows Only). This will only work if accelerated rendering
	 * is enabled (3D acceleration). If it's not then Overlay will be used as
	 * the video renderer instead of VMR9. Applies to all files except DVDs.
	 * 
	 * Returns: true if the video renderer is configured to use VMR9, false
	 * otherwise
	 */
	...
	/**
	 * Returns true if the video renderer is configured to use the Default
	 * renderer (Windows Only). This will usually be the Video Mixing Renderer 7
	 * if using Windows XP; but DirectShow ultimately decides this itself.
	 * Applies to all files except DVDs.
	 * 
	 * Returns: true if the video renderer is configured to use the Default
	 * renderer
	 */
	...
	/**
	 * Sets the video renderer to use the default renderer (Windows Only). This
	 * will usually be the Video Mixing Renderer 7 if using Windows XP; but
	 * DirectShow ultimately decides this itself. Applies to all files except
	 * DVDs.
	 */
	...
	/**
	 * Sets the video renderer to use the Video Mixing Renderer 9 (Windows
	 * Only). This will only work if accelerated rendering is enabled (3D
	 * acceleration). If it's not then Overlay will be used as the video
	 * renderer instead of VMR9. Applies to all files except DVDs.
	 */
	...
	/**
	 * Gets the list of DirectShow audio renderer filters that are available on
	 * the system (Windows Only).
	 * 
	 * Returns: the list of DirectShow audio renderer filters that are available
	 * on the system
	 */
	...
	/**
	 * Gets the different input connectors that are available on the system
	 * sound mixer (Windows Only)
	 * 
	 * Returns: the different input connectors that are available on the system
	 * sound mixer
	 */
